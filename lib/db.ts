...
export function getUserSettings(userId: string): AppSettings | null {
  try {
    ...
    return {
      ...
      morphologyApiKey: row.morphologyApiKey || "",
      morphologyUseNiqqudKey: row.morphologyUseNiqqudKey ?? true,
      morphologyModel: row.morphologyModel || "anthropic/claude-sonnet-4",
      morphologyPrompt: row.morphologyPrompt || "",
      morphologyTemperature: 0.2,
